...
def testRbf(k1=1.3):
    dataArr,labelArr = loadDataSet(r'F:\PY\data\testSetRBF.txt')
    ...
    dataArr,labelArr = loadDataSet(r'F:\PY\data\testSetRBF2.txt')
    errorCount = 0
    datMat=mat(dataArr); labelMat = mat(labelArr).transpose()
    m,n = shape(datMat)
    for i in range(m):
...
def testDigits(kTup=('rbf', 10)):
    dataArr,labelArr = loadImages(r'F:\PY\data\digits\trainingDigits')
    ...
    for i in range(m):
        ...
        if sign(predict)!=sign(labelArr[i]): errorCount += 1
    print "the training error rate is: %f" % (float(errorCount)/m)
    dataArr,labelArr = loadImages(r'F:\PY\data\digits\testDigits')
    errorCount = 0
    datMat=mat(dataArr); labelMat = mat(labelArr).transpose()
    m,n = shape(datMat)
    for i in range(m):
